...
<section class="py-16">
  <div>
    <div class="container mx-auto px-4">
      {#if section.title}
        <h2 class="text-3xl md:text-5xl lg:text-6xl font-bold text-center mb-6 max-w-4xl mx-auto">
          {section.title}
        </h2>
      {/if}

      {#if section.description}
        <p class="text-base md:text-lg text-gray-600 text-center mx-auto mb-12 mt-8 max-w-3xl px-4">
          {section.description}
        </p>
      {/if}
    </div>
    ...
    {#if section.processSteps?.data}
      <div bind:this={stepsContainer} class="flex flex-col md:flex-row flex-wrap justify-center gap-0 w-full mx-auto mb-12">
        {#each section.processSteps.data.sort((a, b) => a.attributes.order - b.attributes.order) as step, index}
          <div
            class="flex-1 min-w-full md:min-w-[150px] px-6 md:px-8 py-8 md:py-12 text-center font-medium text-lg md:text-xl transition-all duration-500 ease-out origin-left
