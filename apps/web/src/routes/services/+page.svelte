...
<section class="relative min-h-screen flex items-center overflow-hidden">
  ...
  <div class="absolute inset-0 z-0">
    ...
    <div class="absolute inset-0 bg-primary-500/30" />
  </div>

  <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-8 w-full">
    <ScrollReveal animation="fade-up" duration="1000">
      <div class="max-w-3xl">
        <span class="inline-block py-2 px-5 rounded-full bg-secondary-500/50 border border-secondary-400/40 text-white text-xs font-bold tracking-[0.2em] uppercase mb-8 backdrop-blur-sm">
          {hero.badge}
        </span>
        <h1 class="text-5xl md:text-7xl lg:text-8xl font-bold text-white mb-6 tracking-tight leading-[1.05]">
          {hero.title}<br/>
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-secondary-500 to-secondary-400">{hero.titleHighlight}</span>
        </h1>
...
<section class="py-24 bg-background-500 overflow-hidden">
    ...
    {#each services as service, idx}
      <div class="flex flex-col {service.imagePosition === 'right' ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-12 lg:gap-24">
        ...
        <ScrollReveal animation={service.imagePosition === 'right' ? 'fade-right' : 'fade-left'} duration="1000">
          <div class="relative group rounded-3xl shadow-2xl shadow-primary-500/10 aspect-[4/3] transform transition-transform duration-700">
            <!-- Skeleton Loader -->
            ...
            {/if}
            <div class="absolute inset-0 bg-primary-500/10 group-hover:bg-transparent transition-colors duration-500 z-10 rounded-3xl"/>
            <img 
              src={service.image}
              alt={service.title}
            ...
            />
            <!-- Floating Icon Card -->
            <div class="absolute bottom-4 right-4 md:bottom-auto md:right-auto {service.imagePosition === 'right' ? 'md:-bottom-6 md:-left-6 lg:md:bottom-8 lg:md:-left-8' : 'md:-bottom-6 md:-right-6 lg:bottom-8 lg:-right-8'} bg-primary-500 p-4 md:p-6 rounded-2xl shadow-[0_20px_50px_rgba(11,_63,_58,_0.3)] z-20 border border-primary-400">
              {#if service.icon === 'search'}
                <svg class="w-8 h-8 md:w-10 md:h-10 text-secondary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <ScrollReveal animation={service.imagePosition === 'right' ? 'fade-left' : 'fade-right'} duration="1000" delay="200">
          <div class="{service.imagePosition === 'right' ? 'md:pr-4' : 'md:pl-4'}">
            <h3 class="text-secondary-700 font-bold tracking-widest uppercase text-[1.5rem] mb-4">{service.category}</h3>
            ...
            <ul class="space-y-4">
              {#each service.features as feature}
                <li class="flex items-center gap-3 text-gray-700 font-medium group">
                  <div class="p-2 rounded-full bg-primary-500 group-hover:bg-primary-600 transition-colors">
...
<section class="relative py-32 px-6 md:px-8 bg-primary-900 text-white overflow-hidden">
  ...
  <div class="relative z-10 max-w-5xl mx-auto text-center">
    <ScrollReveal animation="fade-up" duration="1000">
      <div class="inline-flex items-center justify-center p-4 rounded-full bg-secondary-500/10 border border-secondary-500/20 mb-10 backdrop-blur-sm animate-pulse">
...
<!-- Who We Work With -->
<section class="py-24 px-6 md:px-8 bg-background-300 overflow-hidden">
  <div class="max-w-7xl mx-auto">
    <ScrollReveal animation="fade-up" duration="1000">
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-16">
        <div>
          <h2 class="text-sm font-bold tracking-widest text-secondary-500 uppercase mb-2">{stakeholders.badge}</h2>
    ...
    <ScrollRevealStagger itemCount={stakeholders.items.length} staggerDelay={100} animation="fade-up" let:getItemClass>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {#each stakeholders.items as stakeholder, idx}
          <div class="group h-full bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-primary-500/10 transition-all duration-500 relative transform hover:-translate-y-2 {getItemClass(idx)}">
            <div class="h-64 overflow-hidden relative">
              ...
              <div class="absolute inset-0 bg-primary-500/20 group-hover:bg-transparent transition-colors duration-500 z-10"/>
              <img 
              ...
              <div class="absolute bottom-4 left-4 z-20 text-white">
                {#if stakeholder.icon === 'building'}
                  <svg class="text-secondary-500 w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Closing CTA -->
<section class="py-24 px-6 md:px-8 bg-background-500 relative overflow-hidden">
  <div class="max-w-4xl mx-auto text-center relative z-10">
    <ScrollReveal animation="fade-up" duration="1000">
      ...
      <p class="text-xl md:text-2xl text-gray-600 leading-relaxed mb-10 font-light">
        "{cta.description.split(cta.highlight)[0]}<span class="text-secondary-700 font-medium">{cta.highlight}</span>{cta.description.split(cta.highlight)[1]}"
      </p>
      <a href={cta.buttonLink} class="text-lg inline-flex items-center gap-2 text-primary-700 font-semibold border-b-2 border-primary-700 pb-1 hover:text-secondary-800 hover:border-secondary-800 transition-colors cursor-pointer group">
        {cta.buttonText}
